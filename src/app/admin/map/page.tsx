...
export default function MapPage() {
    ...
    const [searchQuery, setSearchQuery] = useState(''); // <-- TAMBAHKAN BARIS INI
    ...
    const getSafe = (data: any, path: string, fallback: any = 'N/A') => {
        return path.split('.').reduce((acc, part) => acc && acc[part], data) || fallback;
    }

    const filteredData = mapData.filter(item => {
        const namaDonatur = getSafe(item, 'donation_items.donations.nama_donatur', '').toLowerCase();
        const namaPenerima = getSafe(item, 'beswan.nama_beswan', '').toLowerCase();
